...
const RadioButton = ({value, name, onRadioChange, inputProps, children, ...props}: Props) => {
  return (
    <label {...props} className={cn("group flex cursor-pointer items-center gap-10", props.className)}>
      ...
      />
      <span className="realtive m-2 block h-10 w-10 rounded-full border-3 border-white outline-1 outline-black transition-colors peer-checked:bg-black peer-hover:bg-cardinal-red peer-focus-visible:bg-cardinal-red" />
