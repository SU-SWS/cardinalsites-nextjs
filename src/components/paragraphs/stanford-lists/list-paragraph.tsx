...
import {ElementType, HtmlHTMLAttributes, JSX, Suspense} from "react"
import {
  Maybe,
  NodeStanfordCourse,
  NodeStanfordEvent,
...
const loadPage = async (
  ...
  const {items, totalItems} = await getViewPagedItems(viewId, displayId, contextualFilter, PAGE_SIZE, page)
  return (
    <View
      viewId={viewId}
      displayId={displayId}
...
const ListParagraph = async ({paragraph, ...props}: Props) => {
  ...
  const limit = paragraph.suListView?.pageSize || PAGE_SIZE
  ...
  const addLoadMore = (limit || 3) >= PAGE_SIZE && totalItems > viewItems.length
