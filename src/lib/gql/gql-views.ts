...
export const getViewPagedItems = async (
  ...
  return nextCache(
    async (
      _viewId: string,
      _displayId: string,
      _pageSize?: Maybe<number>,
      _contextualFilter?: Maybe<string[]>,
      _page?: Maybe<number>,
      _filter?: ViewFilter
    ) => {
      ...
      try {
        switch (`${viewId}--${displayId}`) {
          ...
          case "stanford_events--past_events_list_block":
            graphqlResponse = await client.request<
              StanfordEventsPastEventsQuery,
              StanfordEventsPastEventsQueryVariables
            >(StanfordEventsPastEventsDocument, {
              contextualFilters,
          ...
          case "stanford_news--spotlight_card_grid_no_date":
            ...
            graphqlResponse = await client.request<StanfordNewsQuery, StanfordNewsQueryVariables>(
              StanfordNewsDocument,
              {
                contextualFilters,
                filter: filters,
          ...
          case "media_filtered--card_grid":
            contextualFilters = getContextualFilters(["term_node_taxonomy_name_depth"], contextualFilter)
            graphqlResponse = await client.request<StanfordMediaQuery, StanfordMediaQueryVariables>(
              StanfordMediaDocument,
              {
                contextualFilters,
                filter,
    ...
    [],
    {tags: ["all-entities", "views", viewTags[viewId] || "views:all"]}
  )(viewId, displayId, pageSize, contextualFilter, page, filter)
}
